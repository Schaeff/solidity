...
size_t AssemblyItem::bytesRequired(size_t _addressLength) const
{
	...
	case VerbatimData:
		return m_verbatimData->second.size();
	default:
		break;
	}
...
string AssemblyItem::toAssemblyText(Assembly const& _assembly) const
{
	...
	case UndefinedItem:
		...
		break;
	case VerbatimData:
		text = string("verbatimdata_") + to_string(m_verbatimData->first) + "_" + util::toHex(m_verbatimData->second);
		break;
	default:
		assertThrow(false, InvalidOpcode, "");
	}
...
ostream& solidity::evmasm::operator<<(ostream& _out, AssemblyItem const& _item)
{
	...
	case VerbatimData:
		// TODO deposit
		_out << " VerbatimData " << util::toHex(_item.verbatimData());
		break;
	case UndefinedItem:
		_out << " ???";
		break;
